...
void Tr_FixedGainCalib_CROC(TString dir, TString filename, int channelAnalyse, double GainSet, double inAmplitude, double inAmplitudeErr, bool saveLvl1, bool saveLvl2, bool mkNewOutputFile, TString outputFileName) {
...
// Plot amplitude graph and its histogram.
	...
	if(plotHistAmpl) {
		...
		cout<<"   Inp_Ampli = "<< setprecision(5)<< inAmplitude*1000 << " +/- "<< setprecision(2)<<inAmplitudeErr*1000<< " mV (" << 100*inAmplitudeErr/inAmplitude<<"%)"<<endl;
		cout<<"   Out_Ampli = " <<setprecision(6)<< meanAmpl<< " +/- "<<setprecision(3)<< sigmaAmpl << " (" << 100*sigmaAmpl/meanAmpl<<"%)"<<endl; 
	...
	if(mkNewOutputFile) {
		...
		*/
		...
		outp <<setprecision(6)<< inAmplitude <<" "<< setprecision(6) <<inAmplitudeErr<< " " << setprecision(8) << meanAmpl<< " "<< setprecision(6)<<meanAmplErr<<std::endl;
