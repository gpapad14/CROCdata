...
void Transp_Noise_Average_CROC(TString dir, TString filename, int channelAnalyse, bool saveLvl1, bool saveLvl2, bool mkOutputFile, TString outputFileName, double GainSet) {
...
// Plot raw graph
	if(plotRaw) {
		...
		grRaw->GetXaxis()->SetTitle("samples");
		...
		if(savecanvas) { cRaw->SaveAs(dir + filename + "_"+channelAnalyseSTR + "_graphZOOM.png",""); }
		delete cRaw;
	}
...
// Make a clean-er dataset.
	...
	for(int i=exclude*PointsPerMeas; i<(exclude+1)*PointsPerMeas; i++) { 
		...
		if( abs(ycoord1+ycoord2+ycoord3-ycoord4-ycoord5-ycoord6)<200 && !found) { // previous limit <110.
			found=true;
			startpoint=i;
		}		
	}
	//if(!found) { delete histRaw; return 0; }
	//cout<<"$ Start point = "<<startpoint<<endl;
	if(startpoint==-1) {
		cout<<"___FATAL ERROR___ StartPoint was not found! ! !"<<endl;
		cout<<"Yet, by hand set to 0."<<endl;
		//delete histRaw;
		//return 0;
		startpoint=0;
	}
	else if( !(startpoint==0) ) {
		cout<<"___ERROR___ StartPoint # 0 ! ! !"<<endl;
	...
	TGraphErrors* grNoise = new TGraphErrors(Nmeasurements*PointsPerMeas-1);

	int binwidthHistNoise=1;
	TH1F* histNoise = new TH1F("histNoise", "Histogram of grNoise", (Ncodes - 0)/binwidthHistNoise, -Ncodes/2, Ncodes/2);
	//int maxAmpl = -1*Ncodes;
	//int minAmpl =  Ncodes;
	double avrdum; // average of 6 points for noise measurements
	int k=0;
	int j;
	for(j=startpoint; j<=Npoints-PointsPerMeas+1; j+=PointsPerMeas){	
		...
		//base[k] = ( p1+p2+p3 )/3;
		//top[k] = ( p4+p5+p6 )/3;
		avrdum = (p1+p2+p3+p4+p5+p6)/6;
		grNoise->SetPoint(j-startpoint, j-startpoint, p1-avrdum);
		histNoise->Fill(p1-avrdum);
		grNoise->SetPoint(j-startpoint+1, j-startpoint+1, p2-avrdum);
		histNoise->Fill(p2-avrdum);
		grNoise->SetPoint(j-startpoint+2, j-startpoint+2, p3-avrdum);
		histNoise->Fill(p3-avrdum);
		grNoise->SetPoint(j-startpoint+3, j-startpoint+3, p4-avrdum);
		histNoise->Fill(p4-avrdum);
		grNoise->SetPoint(j-startpoint+4, j-startpoint+4, p5-avrdum);
		histNoise->Fill(p5-avrdum);
		grNoise->SetPoint(j-startpoint+5, j-startpoint+5, p6-avrdum);
		histNoise->Fill(p6-avrdum);
...
// Plot noise graph and its histogram.
	if(plotNoise) {
		TCanvas* cNoise = new TCanvas("cNoise","cNoise",700,500);
		cNoise->SetGrid();
		cNoise->SetLeftMargin(0.14);
		cNoise->SetRightMargin(0.07);
		grNoise->SetMarkerColor(4);
		grNoise->SetMarkerStyle(7);
		grNoise->GetXaxis()->SetTitle("sample");
		grNoise->GetYaxis()->SetTitle("distance form average (ADU)");
		grNoise->SetTitle("Input noise vs samples");
		grNoise->Draw("AP");
		//grRaw->Fit("fsin","Q0","");
		if(savecanvas) { cNoise->SaveAs(dir + filename + "_"+channelAnalyseSTR + "_grNoise.png",""); }
		delete cNoise;
	}

	double meanNoise, sigmaNoise, sigmaNoiseErr;
	double chi2overNdf;
	if(plotHistNoise) {
		TCanvas* cHistNoise = new TCanvas("cHistNoise","cHistNoise",700,500);
		cHistNoise->SetGrid();
		histNoise->GetXaxis()->SetRangeUser(-200, 200);
		histNoise->GetXaxis()->SetTitle("input noise (ADU)");
		//histNoise->Draw();
		histNoise->Fit("gaus","Q");
		
		TF1* Noisefunc = histNoise->GetFunction("gaus");
		meanNoise = Noisefunc->GetParameter(1);
		sigmaNoise = Noisefunc->GetParameter(2);
		sigmaNoiseErr = Noisefunc->GetParError(2);
		chi2overNdf = Noisefunc->GetChisquare()/Noisefunc->GetNDF();
		histNoise->GetXaxis()->SetRangeUser(meanNoise - 4*sigmaNoise, meanNoise + 4*sigmaNoise);
		//cout<<"   Mean  Noise  = " << meanNoise  <<endl;
		cout<<"   Sigma Noise = " << setprecision(4)<< sigmaNoise <<"         adu"<<endl;
		
		if(saveHistRaw2) { cHistNoise->SaveAs(dir + filename + "_"+channelAnalyseSTR + "_histNoise.png",""); }
		delete cHistNoise;
	}
	delete histNoise;
	...
	double inpNoise = 1e6*sigmaNoise*ADUbeb/abs_gain;
	double inpNoiseErr = inpNoise*sqrt( pow(sigmaNoiseErr/sigmaNoise,2) + pow(ADUbebErr/ADUbeb,2) + pow(abs_gainErr/abs_gain,2) );

	cout<< "   Input noise = "<<setprecision(3)<<inpNoise<<" +/- "<<setprecision(2)<<inpNoiseErr << " uV";
	
	bool errorFound=false;
	//if(chi2overNdf>1.7 || inpNoise>5) { errorFound=true; } 
	if(inpNoise>10) { errorFound=true; } 
